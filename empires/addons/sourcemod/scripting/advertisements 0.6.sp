...
new const String:g_CurrentMapTag[] = "{CURRENTMAP}";
new const String:g_DateTag[] = "{DATE}";
new const String:g_TicrateTag[] = "{TICKRATE}";
new const String:g_TimeTag[] = "{TIME}";
new const String:g_Time24Tag[] = "{TIME24}";
new const String:g_TimeleftTag[] = "{TIMELEFT}";
new const String:g_YouTag[] = "{YOU}";

new const String:CVAR_DISABLED[] = "WYŁ";
new const String:CVAR_ENABLED[] = "WŁ";
new const String:g_DateFormat[] = "%d-%m-%Y";
...
public OnPluginStart()
{
	...
	g_hEnabled        = CreateConVar("sm_advertisements_enabled",  "1",                  "Enable/disable displaying advertisements.", _, true, 0.0, true, 1.0);
	g_hFile           = CreateConVar("sm_advertisements_file",     "advertisements.txt", "File to read the advertisements from.");
	g_hInterval       = CreateConVar("sm_advertisements_interval", "30",                 "Amount of seconds between advertisements.", _, true, 10.0);
...
public Action:Timer_DisplayAds(Handle:timer)
{
	...
	if(StrContains(sText, g_CurrentMapTag) != -1)
	{
		GetCurrentMap(sBuffer, sizeof(sBuffer));
		ReplaceString(sText, sizeof(sText), g_CurrentMapTag, sBuffer);
	}
	
	if(StrContains(sText, g_DateTag)       != -1)
	{
		FormatTime(sBuffer, sizeof(sBuffer), g_DateFormat);
		ReplaceString(sText, sizeof(sText), g_DateTag,       sBuffer);
	}
	
	if(StrContains(sText, g_TicrateTag)   != -1)
	{
		IntToString(g_iTickrate, sBuffer, sizeof(sBuffer));
		ReplaceString(sText, sizeof(sText), g_TicrateTag,   sBuffer);
	}
	
	if(StrContains(sText, g_TimeTag)       != -1)
	{
		FormatTime(sBuffer, sizeof(sBuffer), "%I:%M:%S%p");
		ReplaceString(sText, sizeof(sText), g_TimeTag,       sBuffer);
	}
	
	if(StrContains(sText, g_Time24Tag)     != -1)
	{
		FormatTime(sBuffer, sizeof(sBuffer), "%H:%M:%S");
		ReplaceString(sText, sizeof(sText), g_Time24Tag,     sBuffer);
	}
	
	if(StrContains(sText, g_TimeleftTag)   != -1)
	{
		...
		ReplaceString(sText, sizeof(sText), g_TimeleftTag,   sBuffer);
	}
	...
	decl String:sClientName[MAX_NAME_LENGTH+1];
	new bool:bYou = (StrContains(sText, g_YouTag) != -1);
		...
		for(new i = 1; i <= MaxClients; ++i)
		{
						...
						bAdmins && (GetUserFlagBits(i) & (ADMFLAG_GENERIC|ADMFLAG_ROOT))))
			{
				if(bYou)
				{
					...
					ReplaceString(sBuffer, sizeof(sBuffer), g_YouTag, sClientName);
				}
				PrintCenterText(i, (bYou) ? sBuffer : sText);
				...
				WritePackString(hCenterAd, (bYou) ? sBuffer : sText);
			}
		}
	}
	if(StrContains(sType, "H") != -1)
	{
		for(new i = 1; i <= MaxClients; ++i)
		{
				...
				bAdmins && (GetUserFlagBits(i) & (ADMFLAG_GENERIC|ADMFLAG_ROOT))))
			{
				...
				PrintHintText(i, (bYou) ? sBuffer : sText);
			}
		}
	}
		...
		if(iTeamColors == -1)
		{
			for(new i = 1; i <= MaxClients; ++i)
			{
					...
					bAdmins && (GetUserFlagBits(i) & (ADMFLAG_GENERIC|ADMFLAG_ROOT))))
				{
					...
					PrintToChat(i, (bYou) ? sBuffer : sText);
				}
			}
		}
		else
