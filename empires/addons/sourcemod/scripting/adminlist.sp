...
public Action:Command_AdminOff(client, args)
{
	...
	{
		CReplyToCommand(client, "{Green}[AdminList]{Default} This command must be used in-game and without RCON.");
		return Plugin_Handled;
	}
	hidden[client]=true;
	CReplyToCommand(client, "{Green}[AdminList]{Default} You will no longer show up on the admin list.");
	return Plugin_Handled;
}
...
public Action:Command_AdminPermOff(client, args)
{
	...
	hidden[client]=true;
	CReplyToCommand(client, "{Green}[AdminList]{Default} You have been permanently hidden from the admin list!");
	return Plugin_Handled;
}
...
public Action:Command_AdminOn(client, args)
{
	...
	hidden[client]=false;
	CReplyToCommand(client, "{Green}[AdminList]{Default} You will show up on the admin list again.");
	return Plugin_Handled;
}
	...
	if(GetConVarBool(AdminListEnabled))
	{
		switch(GetConVarInt(AdminListMode))
		{
			case 1:  //Appears in chat area
			{
				...
				if(count==0)
				{
					CPrintToChatAll("{Green}There are no admins online!{Default}");
					return Plugin_Handled;
				}
				else
				{
					...
					CPrintToChatAll("{Green}Admins online are: %s{Default}", buffer);
					return Plugin_Handled;
				}
			}
...
public MenuHandlerAdminList(Handle:menu, MenuAction:action, client, menuPos)
{
	...
	if(action==MenuAction_Select)
	{
		...
		{
			CPrintToChat(client, "{Green}[AdminList]{Default} {Red}ERROR:{Default} Something went horribly wrong with the menu code!");
		}
		CreateMenuAdminList(client);
	}
}